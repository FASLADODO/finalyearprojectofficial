function [personIds,images]=autoEncodeImages(images, personIds, options)
    ...
    hiddenSize2= options.hiddensize2;%int16(0.5*size(sentences,4))
        ...
        if(options.trainLevel==3)
                ...
                imagesTrainIn=cell(size(imagesTrain2,1),1);
                imagesTestIn=cell(size(imagesTest2,1),1);
                imagesAllIn=cell(size(imagesProcess,1),1);
                
                imagesTrainIdsIn=cell(size(imagesIdsTrain2,2),1);
                imagesTestIdsIn=cell(size(imagesIdsTest2,2),1);
                imagesAllIdsIn=cell(size(imagesIdsHot,2),1);
                fprintf('The size of imagesTrainIn is (%d %d)\n', size(imagesTrainIn,1), size(imagesTrainIn,2))  
                % imgheight by 1 cell array Each element X{i,ts} is an width-by-noExamples matrix.
                %T	noClasses by 1 cell array Each element T{i,ts} is a 1-by-noExamples matrix.
                for i=1:size(imagesProcess,1)
                ...
                for i=1:size(imagesTrain2,1)
                   imagesTrainIn{i}=squeeze(imagesTrain2(i,:,:));
                end
                for i=1:size(imagesIdsTrain2,2)
                   imagesTrainIdsIn{i}=squeeze(imagesIdsTrain2(:,i));
                end
                for i=1:size(imagesIdsTest2,2)
                   imagesTestIdsIn{i}=squeeze(imagesIdsTest2(:,i));
                end
                for i=1:size(imagesIdsHot,2)
                   imagesAllIdsIn{i}=squeeze(imagesIdsHot(:,i));
                end

                for i=1:size(imagesTest2,1)
               ...
               % size(sentencesIdsTrain2.')
                ...
                %}
                    ...
                    size(imagesIdsTrain2.')
                    %deepnet.numInputs
                    deepnet = train(deepnet,imagesTrainIn,imagesTrainIdsIn);
                   % deepnet = train(deepnet,imagesTrainIn.',imagesIdsTrain2.');
                    if(size(xTest,2)~=0)
