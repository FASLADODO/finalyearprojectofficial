...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [sentences,sentenceIds]=autoEncodeSentences(sentences, sentenceIds,preciseSentenceIds, netSentences, saveSentences, options)
    ...
    for config = 1:size(sentences,1)
        ...
        if(options.trainLevel==3)
            ...
            if (exist(saveSentences{config}, 'file') ~= 2 || options.force)
                ...
                matchingSentences=sentencesProcess;
                matchingSentencesIds=sentenceIdsProcess;
                if(options.preciseId)
                    [matchingSentencesIds,idx]=sort(preciseSentenceIds);
                    matchingSentences=squeeze(sentences(config,idx,:,:));
                end
                ...
                switch options.sentenceSplit
                    case 'pairs'
                            ...
                            for i=1:length(matchingSentencesIds)
                                if(matchingSentencesIds(i)~=old && occur>1)
                                    for p=1:2%occur
                                       indexes(idx)=i-p;
                                       idx=idx+1;                               
                                    end      
                                    occur=1;
                                    old=matchingSentencesIds(i);
                                else
                                    if(matchingSentencesIds(i)==old)
                                        occur=occur+1;
                                    else
                                       old=matchingSentencesIds(i);
                                       occur=1;
                    ...
                    case 'oneofeach+'
                           ...
                           for i=1:length(matchingSentencesIds)
                            ...
                            indexes= setdiff([1:size(matchingSentences,1)],indexes);
                end       
                %create sentencesTrain and sentencesTest
                %sentneceProcess is all current data in this configfile
                sentencesTrain=matchingSentences(indexes,:,:);
                %sentencesIdsTrain=sentenceIdsProcess(indexes);

                %% NOW CONVERTING SENTENCE IDS TO HOTCODING
                sentencesIdsHot=hotCoding(matchingSentencesIds);
                sentencesIdsTrain=sentencesIdsHot(indexes,:);
                testIndexes= setdiff([1:size(matchingSentences,1)],indexes);
                sentencesTest=matchingSentences(testIndexes,:,:);
