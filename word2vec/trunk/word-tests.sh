...
for t in `seq 0 $NUM`; do
	for w in `seq 0 $WIN`; do
		...
		time ./word2vec -train word-descriptions.txt -output vectors.bin -cbow 1 -size 200 -window ${WINDOWS[$w]} -negative 25 -hs 0 -sample ${SAMPLES[$t]} -threads 20 -binary 1 -iter 15
